...
private fun FarmDetailScreen(
    ...
    Scaffold(
        ...
        Column(
            ...
            OutlinedButton(onClick = onOpenLocation, modifier = Modifier.fillMaxWidth()) {
                Text("Check Location & Sensors")
            }
            OutlinedButton(onClick = { /* Phase 4: start planting session */ }, modifier = Modifier.fillMaxWidth()) {
                Text("Start Planting (Next Phase)")
            }
        }
    }
    if (showAddCrop) {
    ...
    Scaffold(
        ...
        Column(
            ...
            if (!permissionGranted) {
                ...
                HorizontalDivider()
                Text(
                    "Keep the phone still for a few seconds to observe heading stability. Walk slowly to test movement bearing. GPS accuracy is an uncertainty estimate, not a guarantee of exact position.",
