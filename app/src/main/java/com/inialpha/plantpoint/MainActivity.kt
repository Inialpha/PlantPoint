...
private fun FarmDetailScreen(farm: FarmEntity, onBack: () -> Unit) {
    ...
    if (activeCrop != null) {
        PlantingNavigationScreen(farm = farm, crop = activeCrop, onBack = { plantingCrop = null })
        return
    }

    Scaffold(topBar = {
        TopAppBar(title = { Text(farm.name) }, navigationIcon = { TextButton(onClick = onBack) { Text("Back") } })
    }) { padding ->
        Column(modifier = Modifier.fillMaxSize().padding(padding).padding(20.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            crops.forEach { crop ->
                CropRow(crop = crop, onDelete = { farmViewModel.deleteCrop(crop) }, onStartPlanting = { plantingCrop = crop })
            }
...
private fun PlantingNavigationScreen(farm: FarmEntity, crop: CropEntity, onBack: () -> Unit) {
    ...
    Scaffold(topBar = {
        TopAppBar(title = { Text("Plant ${crop.name}") }, navigationIcon = { TextButton(onClick = { locationViewModel.stop(); onBack() }) { Text("Back") } })
    }) { padding ->
        Column(modifier = Modifier.fillMaxSize().padding(padding).padding(20.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            if (location == null) {
                ...
                Button(onClick = {
                    ...
                    plantingViewModel.createStartingPoint(farmId = farm.id, cropId = crop.id, latitude = current.latitude, longitude = current.longitude)
                }, modifier = Modifier.fillMaxWidth()) { Text("Set Starting Point") }
                ...
                if (arrived) {
                    ...
                    Button(onClick = {
                        val current = location ?: return@Button
                        plantingViewModel.recordPlantedTarget(farmId = farm.id, cropId = crop.id, latitude = selectedTarget.latitude, longitude = selectedTarget.longitude, actualLatitude = current.latitude, actualLongitude = current.longitude)
                    }, modifier = Modifier.fillMaxWidth()) { Text("MARK PLANTED") }
                } else {
                ...
                CardinalDirection.entries.forEach { direction ->
                    ...
                    OutlinedButton(onClick = { selectedDirection = direction }, modifier = Modifier.fillMaxWidth()) {
...
private fun LocationDiagnosticsScreen(onBack: () -> Unit) {
    ...
    Scaffold(topBar = {
        TopAppBar(title = { Text("Location Diagnostics") }, navigationIcon = { TextButton(onClick = { locationViewModel.stop(); onBack() }) { Text("Back") } })
    }) { padding ->
